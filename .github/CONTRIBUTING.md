...
runtime in order to build packer with the Hyperv plugin.
...
## Setting up Hyperv plugin for dev

With Go installed, you can already `go get` the Hyperv plugin and `make dev` in
...
1. Download the Hyperv plugin source (and its dependencies) by running
...
4. After building the Hyperv plugin successfully, use the latest version of Packer to build a machine and verify your changes. In the [example folder](https://github.com/hashicorp/packer-plugin-hyperv/blob/main/example) we provide a basic template. Comment out the `packer {}` block to force Packer use the development binary installed in the previous step.
...
need to [fork the Hyperv plugin](https://github.com/hashicorp/packer-plugin-hyperv#fork-destination-box), push your
...
3. One of Hyperv plugin's core team members will look over your contribution and
...
The easiest way to work on a fork is to set it as a remote of the the Hyperv plugin
project. After following the steps in "Setting up Go to work on the Hyperv plugin":
...
Please only apply the minimal vendor changes to get your PR to work. The Hyperv plugin
