# Contributing to Hyper-V Plugin
...
runtime in order to build packer with the Hyper-V plugin.
...
## Setting up Hyper-V plugin for dev

With Go installed, you can already `go get` the Hyper-V plugin and `make dev` in
...
1. Download the Hyper-V plugin source (and its dependencies) by running
   `go get github.com/hashicorp/packer-plugin-hyperv`. This will download the source to
   `$GOPATH/src/github.com/hashicorp/packer-plugin-hyperv`.

2. When working on the Hyper-V plugin, first `cd $GOPATH/src/github.com/hashicorp/packer-plugin-hyperv`
...
3. Make your changes to the Hyper-V plugin source. You can run `make dev` to build and install locally, and `make test` to run unit tests.
...
4. After building the Hyper-V plugin successfully, use the latest version of Packer to build a machine and verify your changes. In the [example folder](https://github.com/hashicorp/packer-plugin-hyperv/blob/main/example) we provide a basic template. Comment out the `packer {}` block to force Packer use the development binary installed in the previous step.
...
need to [fork the Hyper-V plugin](https://github.com/hashicorp/packer-plugin-hyperv#fork-destination-box), push your
...
# Develop a patch.
git push https://github.com/myuser/packer-plugin-hyperv f-my-feature
```
...
3. One of Hyper-V plugin's core team members will look over your contribution and
...
The easiest way to work on a fork is to set it as a remote of the the Hyper-V plugin
project. After following the steps in "Setting up Go to work on the Hyper-V plugin":

1. Navigate to the code:

   `cd $GOPATH/src/github.com/hashicorp/packer-plugin-hyperv`
...
2. Add the remote by running:
   ...
   `git remote add myuser https://github.com/myuser/packer-plugin-hyperv.git`
...
Please only apply the minimal vendor changes to get your PR to work. The Hyper-V plugin
...
If the Hyper-V Plugin has [acceptance tests](https://en.wikipedia.org/wiki/Acceptance_testing), these probably have some requirements such as environment variables to be set for API tokens and keys. Each test should error and tell you what are missing, so those are not documented here.
